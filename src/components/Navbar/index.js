...
const Navbar = ({ navbarData, sections }) => {
  ...
  return (
    <Nav className={scrollDirection === 'down' ? 'hidden' : ''} ref={navbarRef}>
      <NavbarContainer>
        <Link to="/mymind" style={{ display: "flex", alignItems: "center", color: "white", cursor: 'pointer', textDecoration: 'none'}}>
            <Span>MyMind</Span>
          </Link>
        ...
        <ButtonContainer>
          <GitHubButton href={data.navbar.github} target="_blank">Github Profile</GitHubButton>
        </ButtonContainer>
        {
          isOpen &&
          <MobileMenu isOpen={isOpen}>
            ...
            <GitHubButton style={{ padding: '10px 16px', background: theme.primary, color: 'white', width: '100%' }} href={data.navbar.github} target="_blank">
              Github Profile
            </GitHubButton>
          </MobileMenu>
        }
